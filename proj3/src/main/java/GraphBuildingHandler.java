...
public class GraphBuildingHandler extends DefaultHandler {
    ...
    private Stack<GraphDB.Node> tempWay = new Stack<>();
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        if (qName.equals("node")) {
            ...
            activeState = "way";
            long id = Long.valueOf(attributes.getValue("id"));
            GraphDB.Edge way = new GraphDB.Edge(id);
            currentWay = way;
        } else if (activeState.equals("way") && qName.equals("nd")) {
            /* While looking at a way, we found a <nd...> tag. */
            long id = Long.valueOf(attributes.getValue("ref"));
            GraphDB.Node node = g.nodeMap.get(id);
            tempWay.add(node);
            ...
            if (k.equals("maxspeed")) {
                currentWay.setMaxSpeed(v);
            } else if (k.equals("highway")) {
                if (ALLOWED_HIGHWAY_TYPES.contains(v)) {
                    currentWay.isHighway = true;
                }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        if (qName.equals("way")) {
            if (currentWay.isHighway == true) {
                if(!tempWay.isEmpty()){
                    GraphDB.Node n1 = tempWay.pop();
                    while (tempWay.size() > 0) {
                        g.addEdge(n1, tempWay.peek());
                        n1 = tempWay.pop();
                    }
                }
            }
